	...
	<div class="col-span-3 md:col-span-2 space-y-4 pb-4">
		<div class="flex items-center gap-x-2">
			...
			<Sheet.Root>
				<Sheet.Trigger asChild let:builder={sheetBuilder}>
					<Tooltip.Root>
						<Tooltip.Trigger asChild let:builder={tooltipBuilder}>
							<Button
								builders={[tooltipBuilder, sheetBuilder]}
								size="icon"
								variant="outline"
								class="p-2"><Filter /></Button
							>
						</Tooltip.Trigger>
						<Tooltip.Content>
				...
				<Sheet.Content
					side="right"
					class="overflow-y-auto scrollbar-thin scrollbar-thumb-primary scrollbar-track-secondary scrollbar-thumb-rounded"
				>
					<Sheet.Header>
								...
								<Accordion.Content>
									<div class="grid grid-cols-2 gap-y-2">
										<div class="space-y-3">
											<h4>By</h4>
											<RadioGroup.Root
												value={searchBodyData.list_type}
												onValueChange={(value) => {
													searchBodyData.list_type = value;
													loadNextPageManga(true);
												}}
											>
										...
										<div class="space-y-3">
											<h4>Order</h4>
											<RadioGroup.Root
												value={searchBodyData.list_order}
												onValueChange={(value) => {
													searchBodyData.list_order = value;
													loadNextPageManga(true);
												}}
											>
												<div class="flex items-center space-x-2">
								...
								<Accordion.Content>
									<div class="grid grid-cols-2 gap-y-2">
										{#each data.filters.pubstatus as status}
											<div class="flex items-center space-x-2">
												<Checkbox
													checked={searchBodyData.includedLanguages.includes(status.name)}
													id={status.name}
													onCheckedChange={(state) => handleStatusFilter(state, status.name)}
												/>
												<Label
													for={status.name}
								...
								<Accordion.Content>
									<div class="grid grid-cols-2 gap-y-2">
										{#each data.filters.genres as genre}
											<div class="flex items-center space-x-2">
												<Checkbox
													checked={searchBodyData.includedGenres_all.includes(genre.slug)}
													id={genre.slug}
													onCheckedChange={(state) => handleGenreFilter(state, genre.slug)}
												/>
												<Label
													for={genre.slug}
													class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
								...
								<Accordion.Content>
									<div class="grid grid-cols-2 gap-y-2">
										{#each data.filters.themes as theme}
											<div class="flex items-center space-x-2">
												<Checkbox
													checked={searchBodyData.includedGenres_all.includes(theme.slug)}
													id={theme.slug}
													onCheckedChange={(state) => handleGenreFilter(state, theme.slug)}
												/>
												<Label
													for={theme.slug}
													class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
								...
								<Accordion.Content>
									<div class="grid grid-cols-2 gap-y-2">
										{#each data.filters.languages as lang}
											<div class="flex items-center space-x-2">
												<Checkbox
													checked={searchBodyData.includedLanguages.includes(lang.name)}
													id={lang.name}
													onCheckedChange={(state) => handleLangFilter(state, lang.name)}
												/>
												<Label
													for={lang.name}
													class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
				...
				<Tabs.Root value="readCount">
					...
					<Tabs.Content value="bookmarks">
						<TopMangaListViewCard
							...
							mangaList={data.trending.topMangaByBookmarks}
						/>
					</Tabs.Content>
				</Tabs.Root>
			</Card.Content>
