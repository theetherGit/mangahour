    ...
    <div class="grid md:grid-cols-3 gap-5">
        <div class="md:col-span-2">
            <div class="flex items-center justify-between">
                <h2 class="text-xl md:text-3xl font-semibold tracking-tight transition-colors">Manga Read History</h2>
                <Button on:click={() => {deleteAllReadHistory()}} variant="destructive" size="sm">Delete All</Button>
            </div>
            {#if $mangaReadHistory && $mangaReadHistory.length && userReadHistory}
                <div class="grid gap-5 mt-5">
                    {#each $mangaReadHistory as manga}
                        ...
                        <Card.Root class="p-4">
                            <div class="flex items-center justify-between">
                                <h3 class="md:text-xl font-semibold tracking-tight">{slugToTitle(lastReadChapter?.mangaSlug)}</h3>
                                <Button on:click={() => {deleteReadHistorySingle(manga.id)}} variant="destructive" size="sm">Delete History</Button>
                            </div>
                            <Card.Root class="mt-2 p-2 border-none">
                                <h3 class="md:text-xl font-semibold tracking-tight flex justify-between">Last Read Chapter <span>{lastReadChapter.chapterNumber}</span></h3>
                            </Card.Root>
                            <Card.Root class="mt-2 p-2 border-none">
                                <h3 class="md:text-xl font-semibold tracking-tight flex justify-between">Total Read Chapters <span>{manga.chapters.length}</span></h3>
                            </Card.Root>
                        </Card.Root>
                    {/each}
                </div>
            {:else }
                <Alert.Root class="mt-5">
                    ...
                    <Alert.Description>
                        Read some manga on mangahour and check back.
                    </Alert.Description>
                </Alert.Root>
            {/if}
        </div>
        ...
        <div class="grid gap-y-4">
            <div class="flex items-center justify-between">
                <h2 class="text-xl md:text-3xl font-semibold tracking-tight transition-colors">Favourite Manga</h2>
                <Button on:click={() => {deleteAllFavourites()}} variant="destructive" size="sm">Remove All</Button>
            </div>
            {#if $favouriteMangas && $favouriteMangas.length}
